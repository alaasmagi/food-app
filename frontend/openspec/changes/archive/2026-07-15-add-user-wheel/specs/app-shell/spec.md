...
The application SHALL provide a minimal dark-theme layout with a header showing the app name, primary navigation links (Dashboard, Wheel) using Vue Router, and a logout button built from the ported `Button` and `Icon` components. The shell SHALL default to the design system's dark theme with no light-mode toggle. A guarded `/wheel` route SHALL be registered alongside the dashboard route.

#### Scenario: Header renders app name, nav, and logout

- **WHEN** an authenticated user views any shell-wrapped route
- **THEN** the header shows the app name, navigation links to the dashboard and the wheel, and a logout button using the ported `Button` and `Icon`
