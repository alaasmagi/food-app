...
The application SHALL provide a Pinia `auth` store exposing `token`, `currentUser`, `isAuthenticated`, and `hasRole(role)`, plus `login()`, `logout()`, and `fetchToken()` actions. The token SHALL be stored in memory only and never written to `localStorage` or `sessionStorage`, so it does not survive a full page reload by design.
