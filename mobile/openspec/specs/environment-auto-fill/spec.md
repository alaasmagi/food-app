# environment-auto-fill Specification

## Purpose

This capability adds an optional map-based location and radius picker to the dining environment editor, letting a user attach an auto-fill origin (latitude, longitude, and radius in meters) to an environment. It covers surfacing and round-tripping the saved origin through the mobile `DiningEnvironment` type and the create/update write path, the client-side validation mirror of the backend write rules, the `autoFillEnvironment` API call and its React Query mutation hook, and the "Fill with nearby restaurants" action that imports nearby restaurants into the environment and reports the outcome via a summary toast.
