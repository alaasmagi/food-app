# dining-environment-auto-fill Specification

## Purpose
Lets a user store an auto-fill origin (latitude, longitude, optional radius) on a `DiningEnvironment`
and trigger a one-shot, owner-scoped proximity import that adds every restaurant within the radius as an
`EnvironmentRestaurant` membership. Covers the optional fields, their write-path validation, and the
additive auto-fill endpoint. The client supplies coordinates (no server-side geocoding); auto-fill is
explicit and re-runnable, never a live rule.
## Requirements
